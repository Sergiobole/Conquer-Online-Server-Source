...
{
    public class FirePoleWar
    {
        public class GuildWarScrore
        {
            public const int ConquerPointsReward = 50000, FinishMinutes = 15;
        ...
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Server.ServerMaps[Map].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 827));
        }
        ...
        internal unsafe void CompleteEndGuildWar()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                if (Winner.Name != "None" && Winner.Score != 100)
                    msg = "Congratulations to " + Winner.Name + ", they've won the FirePoleWar with a score of " + Winner.Score.ToString();
                else msg = "FirePoleWar has ended with no winner.";
        ...
        public void CheckUp()
        {
            if ((DateTime.Now.Hour == 06 || DateTime.Now.Hour == 18) && DateTime.Now.Minute == 35 && DateTime.Now.Second < 2)
            {
                if (Mode == ProcesType.Dead)
                {
                    Start();
            ...
            if (Mode == ProcesType.Alive)
            {
                if (DateTime.Now > MsgSchedules._FirePoleWar.StampShuffleScore)
                {
                    MsgSchedules._FirePoleWar.ShuffleGuildScores();
                }
        ...
        internal unsafe void FinishRound()
        {
            ...
            {
                var stream = rec.GetStream();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the FirePoleWar round with a score of " + Winner.Score.ToString() + ""
