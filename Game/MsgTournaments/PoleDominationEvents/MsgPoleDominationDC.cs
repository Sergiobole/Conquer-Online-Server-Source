...
namespace COServer.Game.MsgTournaments
{
    ...
    public class MsgPoleDominationDC
    {
        ...
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Server.ServerMaps[1000].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 832));
        }
        ...
        internal unsafe void SendMapPacket(ServerSockets.Packet packet)
        {
            foreach (var client in Server.GamePoll.Values)
            {
                if (client.Player.Map == 1000 || client.Player.Map == 6001)
                {
                    client.Send(packet);
                }
            }
        ...
        internal unsafe void CompleteEndGuildWar()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                if (Winner.Name != "None" && Winner.Score != 100)
                    msg = "Congratulations to " + Winner.Name + ", they've won the PoleDomination of DC with a score of " + Winner.Score.ToString();
                else msg = "PoleDomination of DC has ended with no winner.";
        ...
        public void CheckUp()
        {
            ...
            {
                if (DateTime.Now > MsgSchedules.PoleDominationDC.StampShuffleScore)
                {
                    MsgSchedules.PoleDominationDC.ShuffleGuildScores();
                }
        ...
        internal unsafe void FinishRound()
        {
            ...
            {
                var stream = rec.GetStream();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the PoleDomination of DC round with a score of " + Winner.Score.ToString() + ""
