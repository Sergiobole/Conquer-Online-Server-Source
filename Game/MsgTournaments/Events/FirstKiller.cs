...
{
    public class FirstKiller
    {
        public const uint Map = 502;
        private ProcesType Mode;
        private DateTime FinishTimer = new DateTime();
        private string Title = "FirstKiller";
        ...
        public void CheckUp()
        {
            ...
            if (DateTime.Now.Hour == 17 && DateTime.Now.Minute == 00 && DateTime.Now.Second < 2)
            {
                if (Mode == ProcesType.Dead)
                {
                    FinishTimer = DateTime.Now.AddMinutes(1);
        ...
        public void AddTop(Client.GameClient client)
        {
            if (WinnerUID == client.Player.UID)
                client.EffectStatus.Add("topfirst", DateTime.Now.AddHours(12));
            foreach (var user in Database.Server.GamePoll.Values)
            {
                if (user.Player.UID != WinnerUID)
                    client.EffectStatus.Remove("topfirst");
            }
        }
    }
}
