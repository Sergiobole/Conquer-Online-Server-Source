    ...
    public class BaseEvent
    {
        ...
        public virtual void SendInvitation()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                Program.SendGlobalPackets.Enqueue(new Game.MsgServer.MsgMessage($"{name} has started, type @pvp to join.", "ALLUSERS", Game.MsgServer.MsgMessage.MsgColor.yellow, Game.MsgServer.MsgMessage.ChatMode.Center).GetArray(stream));
            ...
            if (map == EventManager.freezewar.map)
            {
                        ...
                        .Where(e => e.Player.Map == map && e.Player.Alive).Count() == 1)
                {
                        ...
                        .OrderByDescending(e => e.FreezewarPoints))
                    {
                        ...
                        if (r == 1)
                            pl.Player.ConquerPoints += prize;
                        else if (r == 2)
                            pl.Player.ConquerPoints += (uint)(prize * 0.8);
                        else if (r == 3)
                            pl.Player.ConquerPoints += (uint)(prize * 0.6);
                        pl.SendSysMesage("You got " + prize + " CPs!", (Game.MsgServer.MsgMessage.ChatMode)2005);
                        string prize_string = " ";
                        if (prizes != null)
                            for (int i = 0; i < prizes.Length; i++)
                                if (pl.Inventory.HaveSpace(1))
                                {
                                    using (var rec = new ServerSockets.RecycledPacket())
                                    {
                                        var stream = rec.GetStream();
                                        pl.Inventory.Add(stream, prizes[i], 1);
                                    }
                                    Database.ItemType.DBItem dBItem;
                                    if (Database.Server.ItemsBase.TryGetValue(prizes[i], out dBItem))
                                    {
                                        pl.SendSysMesage("You got an item [" + dBItem.Name + "]", (Game.MsgServer.MsgMessage.ChatMode)2005);
